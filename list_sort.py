...
# return an element of the list if it exists in a string
def find_element_in_string(lst, string):
  for item in lst:
    if item in string:
      return item
  return None
...
def sort_list(unsorted_list):
  split_char = find_element_in_string(split_char_list, unsorted_list[0])
  lst = []
  sorted_list = []
  ...
  for item in lst:
    sorted_list.append(item[0]+split_char+item[1])
...
# print_list(sort_list(lst_to_sort, "."))
# print_list(sort_list(lst_to_sort2, " "))
print_list(sort_list(lst_to_sort))
